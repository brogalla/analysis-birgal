...
done_files  = np.sort(glob.glob(f'{run_dir}animations/frames/sfx_*.png'))
if len(done_files)!=0:
   last_date   = datetime.strptime(done_files[-1].split(f'sfx_')[1].split('.png')[0], '%Y-%m-%d').year
   print(last_date)
...
for file in tqdm.tqdm(gridT_files):
   ...
   for time in range(0,sim.time_counter.size):
       ...
       kwags = {'vmin':-3e-3,'vmax':3e-3,'cmap':cmocean.cm.balance, 'ylim':(150,410), 'xlim':(820,None) } # 200, None
       ...
       fig.savefig(f'{run_dir}animations/frames/sfx_{date}.png', facecolor='white', transparent=False, format='png')
       plt.close(fig)

create_animation(glob.glob(f'{run_dir}animations/frames/sfx*'), out_file=f'{run_dir}animations/animation_downward_salt_flux.mp4')
