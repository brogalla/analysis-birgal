...
gridT_files = glob.glob(f'{run_dir}files/*SBC*')
done_files  = np.sort(glob.glob(f'{run_dir}animations/frames/{varname}_*.png'))
if len(done_files)!=0:
   last_date   = datetime.strptime(done_files[-1].split(f'{varname}_')[1].split('.png')[0], '%Y-%m-%d').year
   print(last_date)
...
for file in tqdm.tqdm(gridT_files):
   ...
   for time in range(0,sim.time_counter.size):
       ...
       kwags = {'vmin':-1e-3,'vmax':1e-3,'cmap':cmocean.cm.balance, 'ylim':(150,410), 'xlim':(820,None) } # 200, None
       ...
       fig.savefig(f'{run_dir}animations/frames/{varname}_{date}.png', facecolor='white', transparent=False, format='png')
       plt.close(fig)

create_animation(glob.glob(f'{run_dir}animations/frames/{varname}_*'), out_file=f'{run_dir}animations/animation_net_upward_water_flux.mp4')
